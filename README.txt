...
-Todas as funções excepto getters/setters normais estão comentados e acompanhados da sua complexidade.
-Para calcular a complexidade, por vezes abreviamos certos parâmetros, p.e. "d" = domain(daddy), sendo "daddy" uma variável;
...
Implementações:
    ...
    Amostra:
        list- Lista de dados
        countTensor -Tensor de 4 dimensões
    Floresta:
        v- Vetor com dimensão igual ao número de atributos, cujos valores são as variáveis-pai de cada índice
    Grafos:
        graph- Vetor Listas (LinkedList) de Adjacência
        dim- número de nós
    Bayes:
        tensor- tensor de 3 dimensões
        floresta- Floresta correspondente
